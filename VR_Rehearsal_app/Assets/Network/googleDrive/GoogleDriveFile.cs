...
partial class GoogleDrive
{
	...
	public IEnumerator ListFolders(string folderId){
		#region Check the access token is expired
		var check = CheckExpiration();
		while (check.MoveNext())
			yield return null;
		...
		string newUri = "";
		if (folderId == "") {
			newUri= "https://www.googleapis.com/drive/v2/files?q='root' in parents and trashed=false";
			//newUri = "https://www.googleapis.com/drive/v2/files/root/children";
		} else {
			newUri= "https://www.googleapis.com/drive/v2/files?q='" + folderId + "' in parents and trashed=false"; 
		}
